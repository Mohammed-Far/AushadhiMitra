...
class MainViewModel : ViewModel() {
    ...
    init {
        // Initialize exactly 4 tablets (Tablet 1 to Tablet 4)
    ...
    fun updateMedicine(slot: SlotID, userName: String, time: String) {
        ...
        if (index != -1) {
            ...
            // Sync Daily Schedule for all 7 days
    ...
    fun triggerAlert(medicine: Medicine, recordId: String) {
        // Prevent re-triggering if already dispensed/taken today
        val record = doseRecords.find { it.id == recordId }
        if (record?.status != DoseStatus.PENDING) return

        currentAlertMedicine.value = medicine
        activeDoseId.value = recordId
        isBuzzerActive.value = true
        addSMSLog(SMSLogType.REMINDER, "REMINDER: Time for ${medicine.userMedicineName} (${medicine.tabletName}) on ${record.dayOfWeek}")
    }
    ...
    fun dispenseMedicine() {
        ...
        // Mark current record as dispensed
        updateRecordStatus(recordId, DoseStatus.DISPENSED)
        isBuzzerActive.value = false
        
        addSMSLog(SMSLogType.CONFIRMATION, "${medicine.tabletName} (${medicine.userMedicineName}) dispensed for ${doseRecords.find { it.id == recordId }?.dayOfWeek}. Waiting for pickup...")
